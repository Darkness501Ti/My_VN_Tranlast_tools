...
def lzss_pack_all_literal(input_bytes: bytes) -> bytes:
    """All-literal Ail-LZSS encoder (no back-references).

    Produces output ~1.125x source size. The Ail game engine accepts this
    encoding while our back-ref encoder (``lzss_pack``) triggers
    'Scenario Code decode error' — the engine appears to validate the LZSS
    stream against an encoder-specific invariant we haven't reverse-
    engineered. All-literal is the engine-safe production path.
    """
    out = bytearray()
    src = 0
    n = len(input_bytes)
    while src < n:
        out.append(0)  # ctl byte: all 8 bits = 0 (every op is a literal)
        for _ in range(8):
            if src >= n:
                break
            out.append(input_bytes[src])
            src += 1
    return bytes(out)


def lzss_pack(input_bytes: bytes) -> bytes:
    """Greedy back-ref LZSS encoder (NOT engine-safe).

    Round-trips through lzss_unpack but the game engine rejects the output.
    Kept for round-trip testing; production code uses lzss_pack_all_literal.
    """
    n = len(input_bytes)
    if n == 0:
        return b''
